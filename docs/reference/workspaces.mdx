...
[modules](../bsr/overview.mdx#modules) to import Protobuf files from other local
...
[BSR](../bsr/overview.mdx) so that the other module can update its dependency
and use it locally, potentially using an `alpha` branch to do so. This workflow
...
Now that `build.roots` are deprecated and removed in `v1`, users are encouraged
...
**1. Workspace modules must not overlap, that is one workspace module can not be
...
from the local [module cache](../bsr/overview.mdx#module-cache). This requires
that the latest change has been pushed to the [BSR](../bsr/overview.mdx) and
that the user has run `buf mod update` to update their dependencies and fetch
the latest change.
...
Suppose you are working on both the `buf.build/acme/petapis` and
