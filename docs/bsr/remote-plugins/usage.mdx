...
> We highly recommend completing [the tour](../../tour/generate-go-code.md) to
...
Plugins are invoked in the order they are specified in your `buf.gen.yaml`, but
each plugin has a per-directory parallel invocation, with results from each
...
[`protoc-gen-go`](https://github.com/protocolbuffers/protobuf-go) is generates
...
plugins:
  ...
  - plugin: buf.build/bufbuild/validate-go
    ...
    out: gen/go
    opt:
      # You almost always want to set this option with protoc-gen-validate
...
[Connect](https://connect.build) is a slim library for building browser and
...
[`protoc-gen-es`](https://gitbub.com/bufbuild/protobuf-es) plugin to generate
