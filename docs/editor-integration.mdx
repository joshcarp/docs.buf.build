...
To use Vim integration `buf` must be [installed](installation.mdx). Using
...
You need to have `buf` [installed](installation.mdx) to use it.
...
Make sure `buf` is [installed](installation.mdx) on your PATH, then configure
